...
export function filterByConcept(quizAttempt, concept = null) {
  ...
  if (concept !== null) {
    questionAttempts = quizAttempt.questionAttempts.filter(
...
export function predictedScore(quizAttempt, concept = null) {
  if (
    ...
    quizAttempt.questionAttempts.length === 0 ||
    !quizAttempt.conceptConfidences ||
    quizAttempt.conceptConfidences.length === 0
  ) {
    return 0;
  }
  ...
  if (concept !== null) {
    ...
    if (conceptConfidence === undefined) {
      return 0;
    }
...
export function wadayanoScore(quizAttempt, concept = null) {
  ...
  const accurateConfidenceCount = questionAttempts.filter(q => q.isConfident === q.isCorrect)
    .length;
...
export function confidenceAnalysis(quizAttempt, concept = null) {
  ...
  for (let i = 0; i < questionAttempts.length; i++) {
    const { isConfident, isCorrect } = questionAttempts[i];
  ...
  const wadayano = wadayanoScore(quizAttempt, concept);
