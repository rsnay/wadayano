 ...
 */
const DashboardList = ({ studentQuery }) => {
  if (studentQuery && !studentQuery.currentStudent) {
    return <LoadingBox />;
  }

  if (studentQuery && studentQuery.error) {
    return (
      <ErrorBox>
  ...
  const { courses } = studentQuery.currentStudent;
  const studentName = studentQuery.currentStudent.name;
