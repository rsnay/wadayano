...
class QuestionTaker extends Component {
  ...
  handleKeyDown(e) {
    ...
    if (this.props.question && this.props.question.options) {
      ...
      const questionOptions = this.props.question.options.filter(
  ...
  render() {
          ...
          >
            <button
              className={`column is-1 question-option-letter level-left is-rounded button ${
                this.state.selectedOption && this.state.selectedOption.id === option.id
                  ? 'is-link'
                  : ''
              }`}
            >
      ...
      <div className="columns">
        <div className="column is-two-thirds-tablet field">
          <input
            ...
            onChange={e => {
              this.setState({ shortAnswer: e.target.value });
            }}
          />
        </div>
      </div>
    ...
    const review = this.state.questionAttempt && (
      <QuestionReview questionAttempt={this.state.questionAttempt} question={this.props.question} />
    );

    const continueButton = (
      <button autoFocus className="button is-primary is-medium" onClick={this.props.onNextQuestion}>
        Continue
      </button>
    );
...
export default graphql(ATTEMPT_QUESTION_MUTATION, { name: 'attemptQuestionMutation' })(
  QuestionTaker
);
