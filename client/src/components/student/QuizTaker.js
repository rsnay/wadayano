...
class QuizTaker extends Component {
  ...
  async componentDidMount() {
    ...
    try {
      ...
      // Edge case—if a student answered all questions but didn't click continue after the last question
      // (where the grade is actually submitted), submit it now
      ...
      const randomizedQuestions = shuffleArray(quizAttempt.id, [...quiz.questions]);
      ...
      case phases.CONCEPTS:
        ...
        currentView = (
          <ConceptRater
            ...
            onConceptsRated={() => this.onConceptsRated()}
          />
        );
        break;
      ...
      case phases.QUESTIONS:
        currentView = (
          <QuestionTaker
            ...
            onQuestionCompleted={() => this.onQuestionCompleted()}
        ...
        <div className="container">
          ...
          <div className="columns is-hidden-mobile">
            ...
            <div className="column no-select" style={{ margin: '1rem 0 0 0' }}>
              {this.state.phase === phases.QUESTIONS && (
                <div className="is-flex-tablet">
                  <div style={{ margin: '-.3rem 1rem .5rem 0', flexShrink: 0 }}>
                    Question {this.state.currentQuestionIndex + 1} of {quiz.questions.length}
                  </div>
                  <progress
                    className="progress is-link"
                    value={
                      this.state.currentQuestionIndex +
                      (this.state.currentQuestionCompleted ? 1 : 0)
                    }
          ...
          {this.state.phase === phases.QUESTIONS && (
            ...
            >
              <div>
                {this.state.currentQuestionIndex + 1} of {quiz.questions.length}
              </div>
            </div>
          )}
