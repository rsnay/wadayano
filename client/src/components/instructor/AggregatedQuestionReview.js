...
const optionListing = (
  ...
  return (
    ...
    >
      ...
      {/* Only use dangerouslySetInnerHTML if necessary. For short answers, just show text. */}
      {text ? (
        <span
...
const AggregatedQuestionReview = ({ courseId, quizId, question }) => {
  const [showAllAnswers, setShowAllAnswers] = useState(false);

  const { error, data } = useQuery(COURSE_QUERY, {
  ...
  // Since we use cache-and-network, check for no data (rather than checking if loading)
  ...
  const { correctShortAnswers } = question;
  ...
  if (!isMultipleChoice && !showAllAnswers && answerCounts.size > SHORT_ANSWERS_TO_DISPLAY) {
  ...
  if (isMultipleChoice) {
    ...
    answerCounts.forEach((count, answer) => {
      ...
      optionsView.push(
        optionListing(answer, '', iconClass, icon, null, answer, count, totalAnswerCount)
      );
    });
  }
...
export default AggregatedQuestionReview;
