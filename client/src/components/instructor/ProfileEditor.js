...
class ProfileEditor extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (!nextProps.instructorQuery.error && !nextProps.instructorQuery.loading && !this.state.instructorLoaded) {
            this.setState({
    ...
    async updateProfile() {
        ...
        if (this.state.newPassword.length > 0 && (this.state.newPassword !== this.state.newPasswordConfirm)) {
            this.setState({ error: 'New passwords do not match.', isLoading: false });
            return;
        }
        
        // Send update profile mutation
        const id = this.props.instructorQuery.currentInstructor.id;
    ...
    render() {
        ...
        if (this.props.instructorQuery && this.props.instructorQuery.error) {
            return <ErrorBox><p>Error loading profile. Please try again later.</p></ErrorBox>;
        }
        ...
        const formCompleted = currentPassword && ((newPassword || newPasswordConfirm) ? 
                                                    (newPasswordConfirm === newPassword) : true);
        ...
        <section className="section no-select container">
            ...
            <p className="content">If you need to delete your instructor account, please <Link to="/feedback">contact us</Link>.</p>
...
mutation instructorUpdateProfileMutation ($id: ID!, $currentPassword: String!, $newEmail: String, $newPassword: String) {
    instructorUpdateProfile(id: $id, currentPassword: $currentPassword, newEmail: $newEmail, newPassword: $newPassword)
}`;
