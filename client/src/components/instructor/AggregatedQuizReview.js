...
class AggregatedQuizReview extends Component {
  ...
  processData(nextProps) {
    ...
    const conceptAverageScores = new Map(concepts.map(c => [c, 0]));
    const conceptAveragePredictedScores = new Map(concepts.map(c => [c, 0]));
    const conceptAverageWadayanoScores = new Map(concepts.map(c => [c, 0]));
  ...
  render() {
    if (this.state.error || (this.props.quizQuery && this.props.quizQuery.error)) {
      return (
        <ErrorBox>
          <p>Couldn’t load quiz report. {this.state.error}</p>
        </ErrorBox>
      );
    }
    ...
    return (
      <div>
        ...
        >
          {quiz.questions
            .filter(q => q.concept === this.state.showConceptModal)
            .map(conceptQuestion => (
